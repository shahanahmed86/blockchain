(function () {
...
})()(
	{
		1: [
					...
					var contract = (function (module) {
												...
												var make_attempt = function () {
													C.web3.eth.getTransactionReceipt(tx, function (err, receipt) {
														...
														if (timeout > 0 && new Date().getTime() - start > timeout) {
															return reject(
																new Error('Transaction ' + tx + " wasn't processed in " + timeout / 1e3 + ' seconds!')
															);
														}
						...
						Contract._static_methods = {
							...
							new: function () {
								...
								if (!this.bytecode) {
									throw new Error(
										this._json.contractName + " error: contract binary not set. Can't deploy new instance."
									);
								}
								return self
									.detectNetwork()
											...
											return new Promise(function (accept, reject) {
												self.web3.eth.getCode(address, function (err, code) {
													...
													if (!code || code.replace('0x', '').replace(/0/g, '') === '') {
														return reject(
															new Error(
																'Cannot create instance of ' + self.contractName + '; no code at address ' + address
															)
														);
													}
													accept(instance);
							...
							deployed: function () {
								...
								return self.detectNetwork().then(function () {
									if (self._json.networks[self.network_id] == null) {
										throw new Error(
											self.contractName + ' has not been deployed to detected network (network/artifact mismatch)'
										);
									}
									if (!self.isDeployed()) {
										throw new Error(
											self.contractName + ' has not been deployed to detected network (' + self.network_id + ')'
										);
									}
						...
						Contract._properties = {
							...
							address: {
								get: function () {
									...
									if (address == null) {
										throw new Error(
											'Cannot find deployed address: ' + this.contractName + ' not deployed or address not set.'
										);
									}
									return address;
								},
			...
			{ './statuserror.js': 79, 'ethjs-abi': 48, 'truffle-blockchain-utils': 63, web3: 45 }
		],
		2: [
			function (require, module, exports) {
		...
		11: [
			...
			{ './schema_obj': 13, './util': 15, 'fast-deep-equal': 50, 'json-schema-traverse': 55, url: 74 }
		],
		12: [
			function (require, module, exports) {
				'use strict';
				...
				module.exports = function generate_custom(it, $keyword, $ruleType) {
					...
					if ($isData && $rDef.$data) {
						...
						if ($validateSchema) {
							$closingBraces += '}';
							out +=
								' ' + $valid + ' = ' + $definition + '.validateSchema(' + $schemaValue + '); if (' + $valid + ') { ';
						}
					}
					if ($inline) {
						if ($rDef.statements) {
							...
							if ($noAdditional) {
								if ($removeAdditional) {
									...
									if (it.createErrors !== false) {
										...
										if (it.opts.verbose) {
											out +=
												' , schema: false , parentSchema: validate.schema' + it.schemaPath + ' , data: ' + $data + ' ';
										}
										out += ' } ';
									} else {
										out += ' {} ';
						...
						if (arr6) {
							...
							while (i6 < l6) {
								...
								if (it.util.schemaHasRules($sch, it.RULES.all)) {
									...
									$it.errSchemaPath =
										it.errSchemaPath + '/patternGroups/' + it.util.escapeFragment($pgProperty) + '/schema';
			...
			function (require, module, exports) {
				var Blockchain = {
					getBlockByNumber: function (blockNumber, provider, callback) {
						...
						provider.sendAsync(
							{ jsonrpc: '2.0', method: 'eth_getBlockByNumber', params: params, id: Date.now() },
							callback
						);
					},
					getBlockByHash: function (blockHash, provider, callback) {
						...
						provider.sendAsync(
							{ jsonrpc: '2.0', method: 'eth_getBlockByHash', params: params, id: Date.now() },
							callback
						);
					},
					parse: function (uri) {
		...
		70: [
			function (require, module, exports) {
				module.exports = {
					...
					properties: {
						contractName: {
							allOf: [
								...
								{ description: 'Name used to identify the contract', default: 'Contract' }
							]
						},
						abi: {
							allOf: [
						...
						deployedSourceMap: {
							allOf: [{ $ref: '#/definitions/SourceMap' }, { description: 'Source mapping for contract bytecode' }]
						},
					...
					required: ['abi'],
					patternProperties: {
						'^x-': { anyOf: [{ type: 'string' }, { type: 'number' }, { type: 'object' }, { type: 'array' }] }
					},
					additionalProperties: false,
					definitions: {
